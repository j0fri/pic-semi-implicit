...
Config<T,2,3> preset_configs::landau2D3VX(unsigned int Nx, unsigned int Ny){
    Config<T,2,3> config{
        std::vector<typename Config<T,2,3>::SpeciesConfig>{{
               preset_species::Uniform2D3V<T>(50000,1,-1,4*M_PI,1,Nx,Ny,1,0.001),
               preset_species::Uniform2D3V<T>(50000,2000,1,4*M_PI,1,Nx,Ny,1,0.001),
        }},
...
Config<T,2,3> preset_configs::landau2D3VXWave(unsigned int Nx, unsigned int Ny){
    ...
    config.fieldConfig.forcedE[0] += preset_distributions::Sin<T,2>(0,1,(T)1.0/2,0);
