...
if Ns > 1
    disp("Pick which species to plot in si, 1-indexed");
end
si = 1;
...
data = readmatrix("../outputs/speciesPositionDistribution.txt",'NumHeaderLines',0);
lda = (Nx+1)*Ns;
Nt = size(data,1)/lda;
distribution = cell(Nt,1);

for i = 1:Nt
    distribution{i}=data(lda*(i-1)+(1+Nx)*(si-1)+2:lda*(i-1)+(1+Nx)*(si-1)+1+Nx, 1:Ny);
end
